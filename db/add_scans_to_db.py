...
def add_scans_to_db(scans_data: List[Dict], save_to_file: Optional[str] = None, save_to_db: bool = True, db_name: str = "AnimeSama", collection_name: str = "scans") -> int:
    ...
    Args:
        scans_data: list of scan entries (each entry should contain at least 'title', 'url', and 'chapters')
    ...
    if not isinstance(scans_data, list):
        raise ValueError("scans_data must be a list of scan entries")
    ...
    if save_to_db and scans_data:
        ...
        for entry in scans_data:
            ...
            if not title or not url:
                continue
            # Upsert based on title and url to avoid duplicates
            filter_ = {"title": title, "url": url}
            update = {"$set": entry}
